...
- Bug in :meth:`Series.value_counts` with ``bins`` and ``normalize=True`` including missing and out-of-range values in the normalization denominator when ``dropna=True`` (:issue:`44946`)
